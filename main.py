...
class ATRWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Pace Chaser")
        ...
        version_layout.setContentsMargins(0, 0, 10, 5)
        version_layout.addStretch()
        self.version_label = QLabel(f"V{APP_VERSION}")
        self.version_label.setStyleSheet("color: grey; font-size: 9pt;")
    ...
    def apply_theme(self):
        ...
        if self.theme == "Light":
            ...
            bg_color = "#F5F5F7" # Modern light grey
            fg_color = "#1D1D1F" # Near black
            table_bg = "#FFFFFF"
            table_alt_bg = "#F5F5F7"
            input_bg = "#FFFFFF"
            border_color = "#D1D1D6"
            accent_color = "#007AFF" # iOS Blue-ish
            self.plot_pen = 'b'
            ...
            stylesheet = f"""
                QMainWindow, QWidget {{ background-color: {bg_color}; color: {fg_color}; font-family: "Segoe UI", "Inter", "Helvetica Neue", Helvetica, Arial, sans-serif; font-size: 10pt; }}
                
                /* Tabs */
                QTabWidget::pane {{ border: 1px solid {border_color}; border-radius: 6px; top: -1px; background: {table_bg}; }} 
                QTabBar::tab {{ background-color: transparent; color: {fg_color}; border: none; border-bottom: 2px solid transparent; padding: 8px 16px; margin-right: 2px; font-weight: 500; }}
                QTabBar::tab:selected {{ color: {accent_color}; border-bottom: 2px solid {accent_color}; }}
                QTabBar::tab:hover {{ background-color: rgba(0,0,0,0.05); border-radius: 4px; }}

                /* Table */
                QTableWidget {{ background-color: {table_bg}; alternate-background-color: {table_alt_bg}; color: {fg_color}; gridline-color: transparent; border: 1px solid {border_color}; border-radius: 6px; selection-background-color: {accent_color}; selection-color: white; }}
                QHeaderView::section {{ background-color: {bg_color}; color: {fg_color}; border: none; border-bottom: 1px solid {border_color}; padding: 6px; font-weight: 600; }}
                QTableCornerButton::section {{ background-color: {bg_color}; border: none; }}
                
                /* Inputs & Combos */
                QTextEdit {{ background-color: {input_bg}; color: {fg_color}; border: 1px solid {border_color}; border-radius: 6px; padding: 6px; font-family: 'Consolas', 'Monaco', monospace; }}
                QLineEdit, QComboBox, QDoubleSpinBox {{ background-color: {input_bg}; color: {fg_color}; border: 1px solid {border_color}; border-radius: 6px; padding: 5px; }}
                QLineEdit:focus, QComboBox:focus, QDoubleSpinBox:focus {{ border: 1px solid {accent_color}; }}
                
                QComboBox::drop-down {{ subcontrol-origin: padding; subcontrol-position: top right; width: 20px; border-left: none; }}
                QComboBox::down-arrow {{ image: none; border-left: 4px solid transparent; border-right: 4px solid transparent; border-top: 5px solid #666666; margin-right: 5px; }}
                
                QDoubleSpinBox::up-button, QDoubleSpinBox::down-button {{ width: 16px; border: none; background: transparent; margin: 1px; border-radius: 2px; }}
                QDoubleSpinBox::up-button:hover, QDoubleSpinBox::down-button:hover {{ background: #E5E5EA; }}
                QDoubleSpinBox::up-arrow {{ border-left: 3px solid transparent; border-right: 3px solid transparent; border-bottom: 4px solid #666666; }}
                QDoubleSpinBox::down-arrow {{ border-left: 3px solid transparent; border-right: 3px solid transparent; border-top: 4px solid #666666; }}
                
                /* Buttons */
                QPushButton {{ background-color: {input_bg}; border: 1px solid {border_color}; border-radius: 6px; padding: 6px 16px; min-width: 60px; font-weight: 500; }}
                QPushButton:hover {{ background-color: #F2F2F7; border-color: {accent_color}; }}
                QPushButton:pressed {{ background-color: #E5E5EA; }}
                
                /* Scrollbars */
                QScrollBar:vertical {{ border: none; background: transparent; width: 12px; margin: 0px; }}
                QScrollBar::handle:vertical {{ background: #C1C1C1; min-height: 20px; border-radius: 6px; margin: 2px; }}
                ...
                QTableWidget QComboBox {{ margin: 2px; background-color: {input_bg}; border: 1px solid {border_color}; }}
                QTableWidget QDoubleSpinBox {{ margin: 2px; border: 1px solid {border_color}; }}
            """
        else:
            # Dark Theme Modernized
            bg_color = "#1E1E1E" # Darker, flatter background
            fg_color = "#D4D4D4" # Softer white
            table_bg = "#252526"
            table_alt_bg = "#2D2D2D"
            input_bg = "#3C3C3C"
            border_color = "#3E3E42" # Subtle border
            ...
            self.atr_plot.setBackground('#1E1E1E')
            self.atr_plot.getAxis('bottom').setPen('#858585')
            self.atr_plot.getAxis('left').setPen('#858585')
            ...
            stylesheet = f"""
                ...
                QTabBar::tab {{ background-color: transparent; color: {fg_color}; border: none; border-bottom: 2px solid transparent; padding: 8px 16px; margin-right: 2px; font-weight: 500; }}
                QTabBar::tab:selected {{ color: {accent_color}; border-bottom: 2px solid {accent_color}; }}
                QTabBar::tab:hover {{ background-color: rgba(255,255,255,0.05); border-radius: 4px; }}
                ...
                QComboBox::drop-down {{ subcontrol-origin: padding; subcontrol-position: top right; width: 20px; border-left: none; }}
                QComboBox::down-arrow {{ image: none; border-left: 4px solid transparent; border-right: 4px solid transparent; border-top: 5px solid #CCCCCC; margin-right: 5px; }}
                
                QDoubleSpinBox::up-button, QDoubleSpinBox::down-button {{ width: 16px; border: none; background: transparent; margin: 1px; border-radius: 2px; }}
                QDoubleSpinBox::up-button:hover, QDoubleSpinBox::down-button:hover {{ background: #4C4C4C; }}
                QDoubleSpinBox::up-arrow {{ border-left: 3px solid transparent; border-right: 3px solid transparent; border-bottom: 4px solid #CCCCCC; }}
                QDoubleSpinBox::down-arrow {{ border-left: 3px solid transparent; border-right: 3px solid transparent; border-top: 4px solid #CCCCCC; }}
                
                /* Buttons */
                QPushButton {{ background-color: {input_bg}; border: 1px solid {border_color}; border-radius: 6px; padding: 6px 16px; min-width: 60px; color: {fg_color}; font-weight: 500; }}
                QPushButton:hover {{ background-color: #4C4C4C; border-color: {accent_color}; }}
    ...
    def populate_positions_table(self):
        ...
        for i, p_data in enumerate(self.positions_data):
            try:
                ...
                # Show exact value up to 6 decimals, stripping trailing zeros
                avg_cost_str = f"{avg_cost:,.6f}".rstrip('0').rstrip('.') if avg_cost != 0 else "0.00"
                item_6 = NumericTableWidgetItem(avg_cost_str)
                ...
                price_str = f"{price:,.6f}".rstrip('0').rstrip('.') if price != 0 else "0.00"
                item_7 = NumericTableWidgetItem(price_str)
